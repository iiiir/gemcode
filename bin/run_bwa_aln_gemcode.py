...
p = argparse.ArgumentParser(description='run_bwa_aln_gemcode.py -b *.bam -o o.bam -O folder --tmp /scratch_space -j jobs.sjm')
...
def bam_to_cam(pjobs):
    jobs = []
    for pjob in pjobs:
        bam = pjob.output
        job = sjm.Job('barbam-%s' % bam.name )
        job.memory = "10G"
        job.output = util.File( bam.path.rstrip('.sort.bam') + '.bc.cam' )
        job.append('barbam.sh %s %s' % (bam, job.output) )
        job.depend(pjob)
        jobs.append(job)
    return jobs

def merge_cam(pjobs, outcam):
    jobs = []
    cams = []
    for pjob in pjobs:
        cams.append(pjob.output.path)
    job = sjm.Job('picard_mergeSam-%s' % outcam )
    job.memory = "20G"
    job.output = util.File( os.path.join(outdir, outcam) )
    job.append('picard_msam.sh %s %s'%(job.output, ' '.join(cams) ) )
    job.depend(*pjobs)
    jobs.append(job)
    return jobs
...
# Parallele process to produce cam
jobs_cam = bam_to_cam(jobs)
jobs_cam = merge_cam(jobs_cam, args.output_prefix.rstrip('.bam') + '.cam')

# Continue regular BAM
